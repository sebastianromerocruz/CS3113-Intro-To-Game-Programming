...
constexpr float TILE_DIMENSION = 50.0f, ACCELERATION_OF_GRAVITY = 981.0f,
...
void initialise()
{
    InitWindow(SCREEN_WIDTH, SCREEN_HEIGHT, "AI — Exercise 06");
    InitAudioDevice();
    ...
    */
    ...
    for (int i = 0; i < NUMBER_OF_TILES; i++)
    {
        ...
        gState.tiles[i].setColliderDimensions(
            { TILE_DIMENSION, TILE_DIMENSION }
        );
        gState.tiles[i].setPosition(
            { leftMostX + i * TILE_DIMENSION, ORIGIN.y + TILE_DIMENSION }
        );
    }
    ...
    */
    ...
    // TODO: Call setLerpFactor() on the ghost once you have added that setter.
    //       Try a value of 2.0f to start, then experiment!

    gState.ghost->setColliderDimensions(
        { gState.ghost->getScale().x / 2.0f, gState.ghost->getScale().y }
    );

    // No gravity — this ghost floats freely through the air.
    ...
    gState.ghost->render(); // prime the animation atlas before the first frame
...
void update()
{
    ...
    while (deltaTime >= FIXED_TIMESTEP)
    {
        ...
        for (int i = 0; i < NUMBER_OF_BLOCKS; i++)
            gState.blocks[i].update(
                FIXED_TIMESTEP, nullptr, nullptr, 0, nullptr, 0
            );

        for (int i = 0; i < NUMBER_OF_TILES; i++)
            gState.tiles[i].update(
                FIXED_TIMESTEP, nullptr, nullptr, 0, nullptr, 0
            );
