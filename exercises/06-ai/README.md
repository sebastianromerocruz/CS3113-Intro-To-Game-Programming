...
When applying this to smooth movement, map the variables like this:

| Formula variable | Meaning in your code |
|---|---|
| `a` | The entity's current position |
| `b` | The target's position  |
| `t` | the lerp factor * the delta time |

Substituting those in, what you actually write each frame for **one axis** looks like:

```
position' = position + (target position - position) * lerp factor * delta time
```

There's one other important thing to notice: this approach **bypasses `mMovement` and `mVelocity` entirely**. Take a look at the `update()` method in `Entity.cpp`—you'll see that the normal velocity/collision pipeline is already skipped for `LERPER` entities, precisely so that `AILerp` can drive `mPosition` directly.

Each frame, `(b - a)` (the remaining distance) gets a little smaller, so the step gets a little smaller too. The entity covers a fixed _fraction_ of the gap per frame rather than a fixed _amount_. That shrinking step size is what produces the characteristic ease-out feel: fast at first, then gliding to a stop near the target.
