...
void initialise()
{
    ...
    gLevels[DUNGEON] = new Dungeon(ORIGIN, "#011627");

    switchToScene(gLevels[DUNGEON]);
...
void processInput() 
{
    ...
    Animation hinasCurrAnim = gCurrentScene->getState().hinako->getAnimation();

    if ((IsKeyDown(KEY_S) && hinasCurrAnim != CHARGING) || IsKeyReleased(KEY_S)) 
...
void render()
{
    ...
    gShader.setInt(
        "isCharging", 
        gCurrentScene->getState().hinako->getAnimation() == CHARGING ? 1 : 0
    );
