...
2. If there's a collision detected, we'll use the collidably entity's position and collider dimensions (along with the protagonist's) in order to calculate the amount of overlap between the two. The formula for that is the following:
    ...
    ```
3. If this vertical collision resulted during a positive velocity state (i.e. we "fell" onto the entity we collided, since downwards is positive in raylib), _subtract_ the amount of overlap from the y-component of the player's position.
4. If this vertical collision resulted during a negative velocity state (i.e. we "jumped" to "hit" the entity we collided), _add_ the amount of overlap to the y-component of the player's position.
...
This fix for this is simple: when checking x-collisions, simply skip over those where there's a minuscule y-overlap. Those objects are clearly not in large enough to warrant a collision overlap fix, since they are not at "eye-level". In other words, simply add the following lines to your `checkCollisionX` method:
...
Raw delta time makes physics run at whatever speed the rendering loop happens to achieve, which can vary wildly across hardware or load spikes. At a large enough `Δt`, a fast-moving object can skip past a thin collider entirely in a single frame:
...
<p align=center>
    <sub>
        <strong>Figure XV</strong>: This is what we call "tunnelling" through a wall.
    </sub>
</p>

The fix is a **fixed timestep**: rather than passing the raw frame duration straight into `update`, we accumulate elapsed time and advance the simulation in constant, predefined increments, carrying any sub-step remainder into the next frame.
...
void update()
{
    ...
    deltaTime += gTimeAccumulator;        // fold in any leftover from last frame

    if (deltaTime < FIXED_TIMESTEP)       // not enough time for a full step yet
    {
        gTimeAccumulator = deltaTime;
        return;
    }

    while (deltaTime >= FIXED_TIMESTEP)   // consume as many full steps as possible
    {
        gProtag->update(FIXED_TIMESTEP, gTiles, NUMBER_OF_TILES);
    ...
    gTimeAccumulator = deltaTime;         // save the remainder for next frame
}
```

This decouples render rate from simulation rate: collisions resolve the same way each frame and physics integrators stay numerically stable regardless of how fast (or slow) the machine is running.

You might be wondering to yourself: wait, what's even the difference between the two? It's worth understanding the distinction, since the two are easily conflated.

- **Delta time** makes objects move at a consistent _speed_ by scaling each displacement by how long the frame took—fast machine, slow machine, same apparent velocity. 
- A **fixed timestep** goes one step further: rather than letting `Δt` vary with each call, it ensures the physics update always receives the _same_ `Δt`. 

It's not one or the other. Delta time still feeds the accumulator, so we're still very much grounded on time. The fixed timestep simply guarantees that the math on the other end always sees a well-behaved, constant slice of time rather than whatever the last frame happened to take.
...
<p align=center>
    <sub>
        <strong>Figures XVI, XVII, & XVIII</strong>: In spite of the different FPSs and potential load spikes, all objects are where they're meant to be.
    </sub>
</p>
