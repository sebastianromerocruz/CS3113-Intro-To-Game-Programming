...
constexpr float TILE_DIMENSION  = 50.0f,
    ...
    ACCELERATION_OF_GRAVITY = 981.0f, FIXED_TIMESTEP = 1.0f / 60.0f;
...
void initialise()
{
    ...
    */
    ...
    gXochitl->setColliderDimensions(
        { gXochitl->getScale().x / 3.0f, gXochitl->getScale().y / 3.0f }
    );
    ...
    */
    ...
    gTiles[NUMBER_OF_TILES - 1].setColliderDimensions(
        { TILE_DIMENSION, TILE_DIMENSION }
    );
    gTiles[NUMBER_OF_TILES - 1].setPosition(
        { ORIGIN.y + TILE_DIMENSION, ORIGIN.y - TILE_DIMENSION }
    );
...
void update()
{
    ...
    while (deltaTime >=
           FIXED_TIMESTEP) // consume as many full steps as possible
    {
