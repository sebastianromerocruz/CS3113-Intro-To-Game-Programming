...
<h3 align=center>XXVI Harpstring Moon, Imperial Year MMXXVI</h3>
...
2. [**Textures**](#2)
    ...
    2. [**Textures in raylib**](#2-2)
        1. [**_Determine The Texture's Local (UV) Coordinates_**](#2-2-1)
...
Well, gone are those good old days! While lots of good games have been made with just solid colour shapes, our goal this semester is to make games that use **textures**. A texture is basically any 2D image applied to either 3D models or surfaces that gives them a level of visual detail and realism. Naturally, this process comes with its own bit of setup, so that's what we'll be going through today.
...
1. The **design** itself, independent of your laptop.
2. The _dimensions_ of the design as a **sticker**, and _where_ we want it on your **laptop**.
...
1. The texture's own "local" (UV) coordinates.
...
#### Step 1: _Determine The Texture's Local (UV) Coordinates_
...
> Now that we've created a destination area on the window, what point _within that rectangle_ would you like to consider the _rotational origin_?

In other words, what point within your destination rectangle would you like to pick so that, when we apply the texture on it, that point will be its origin? Here, we can apply that offset that we caused in step 2:
...
void render()
{
    ...
    // Origin inside the destination rectangle (lower-left corner of dest rect)
...
void DrawTexturePro(
    ...
    Vector2 origin,    // the texture origin from step 3
...
<p align=center>
    <img src="assets/README/delta-time-explained.svg">
    </img>
</p>

<p align=center>
    <sub>
        <strong>Figure V</strong>: Without delta time, a 30 FPS machine covers only half the distance a 60 FPS machine covers in the same wall-clock second. Multiplying by <code>Δt</code> gives the slower machine a proportionally larger step each frame, so both machines end up at the same position.
    </sub>
</p>
